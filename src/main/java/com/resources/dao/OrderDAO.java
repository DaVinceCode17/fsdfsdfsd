...
public class OrderDAO {
    ...
    // ===== SAVE TO SET PRICING + INSERT TO LAUNDRY LOGS =====
    public Order saveToSetPricing(Order order) throws SQLException {
        ...
        String logSql = "INSERT INTO laundry_logs (customer_id, services, service_type, queue_number, weight, price, status) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DatabaseConnection.getConnection();
    ...
    // ===== MOVE TO FOR PICKUP - FIXED =====
    public Order moveToForPickup(int queueNumber) throws SQLException {
        String[] tables = {"to_fold_orders", "to_iron_orders", "to_dry_orders", "to_wash_orders", "pending_orders"};
        for (String table : tables) {
            Order order = getOrderByQueueNumber(queueNumber, table);
            if (order != null) {
                return moveOrderByQueueNumber(queueNumber, table, "for_pickup_orders", "for_pickup");
            }
        }
        return null;
    }
    
    // ===== MOVE TO DELIVER - FIXED =====
    public Order moveToDeliver(int queueNumber) throws SQLException {
        String[] tables = {"to_fold_orders", "to_iron_orders", "to_dry_orders", "to_wash_orders", "pending_orders"};
        for (String table : tables) {
            Order order = getOrderByQueueNumber(queueNumber, table);
            if (order != null) {
                return moveOrderByQueueNumber(queueNumber, table, "to_deliver_orders", "to_deliver");
            }
        }
        return null;
    }
    ...
    private Order moveOrderByQueueNumber(int queueNumber, String fromTable, String toTable, String newStatus) throws SQLException {
        // SELECT
        String selectSql = "SELECT o.*, c.first_name, c.last_name, c.address FROM " + fromTable + " o LEFT JOIN customers c ON o.customer_id = c.id WHERE o.queue_number = ?";
        Order order = null;
        try (Connection conn = DatabaseConnection.getConnection();
        ...
        if (toTable.equals("to_deliver_orders")) {
            insertSql = "INSERT INTO to_deliver_orders (customer_id, address, services, service_type, queue_number, weight, price) VALUES (?, ?, ?, ?, ?, ?, ?)";
            try (Connection conn = DatabaseConnection.getConnection();
        ...
        String updateLogSql = "UPDATE laundry_logs SET status = ? WHERE queue_number = ?";
        try (Connection conn = DatabaseConnection.getConnection();
    ...
    public boolean updateSetPricingWeightAndPrice(int queueNumber, double weight, double price) throws SQLException {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (updated) {
                String logSql = "UPDATE laundry_logs SET weight = ?, price = ? WHERE queue_number = ?";
    ...
    public Order getSetPricingByQueueNumber(int queueNumber) throws SQLException {
        String sql = "SELECT o.*, c.first_name, c.last_name, c.address FROM set_pricing_orders o LEFT JOIN customers c ON o.customer_id = c.id WHERE o.queue_number = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, queueNumber);
            ResultSet rs = pstmt.executeQuery();
    ...
    public Order getPendingByQueueNumber(int queueNumber) throws SQLException {
        String sql = "SELECT o.*, c.first_name, c.last_name, c.address FROM pending_orders o LEFT JOIN customers c ON o.customer_id = c.id WHERE o.queue_number = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, queueNumber);
            ResultSet rs = pstmt.executeQuery();
